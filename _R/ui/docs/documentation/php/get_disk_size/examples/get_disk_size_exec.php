...
if($this->oCRNRSTN->grant_permissions_fwrite($tmp_write_path, $tmp_small_data_size)){
    ...
    $tmp_html_out .= 'Permission granted to write ' . $this->oCRNRSTN->return_bytes($tmp_small_data_size, 4) . ' of data.<br><br>';

}else{

    $tmp_html_out .= 'Permission DENIED to write ' . $this->oCRNRSTN->return_bytes($tmp_small_data_size, 4) . '!<br><br>';
...
if($this->oCRNRSTN->grant_permissions_fwrite($tmp_write_path, $tmp_big_data_size)){
    ...
    $tmp_html_out .= 'Permission granted to write ' . $this->oCRNRSTN->return_bytes($tmp_big_data_size, 4) . '!';

}else{

    $tmp_html_out .= 'Permission DENIED to write ' . $this->oCRNRSTN->return_bytes($tmp_big_data_size, 4) . '!';
